...
export const dbService = {
  ...
  async updateUserProfile(db, userId, data) {
    const fields = [];
    const values = [];
  ...
  async updateUserPresence(db, userId, isOnline) {
  ...
  // --- GROUPS ---
  async createGroup(db, creatorId, data) {
    ...
    // 2. Insert into chats table for unified messaging
    await db.prepare(
      "INSERT OR IGNORE INTO chats (id, type, name, avatar_url, created_by, created_at, updated_at) VALUES (?, 'group', ?, ?, ?, ?, ?)"
    ).bind(groupId, data.name, data.avatar_url || null, creatorId, Date.now(), Date.now()).run();
    ...
    await db.prepare(
      "INSERT OR IGNORE INTO chat_participants (id, chat_id, user_id, role, joined_at) VALUES (?, ?, ?, 'admin', ?)"
    ).bind(crypto.randomUUID(), groupId, creatorId, Date.now()).run();

    // 4. Add initial members (capped at 50)
    ...
    for (const memberId of membersToAdd) {
      ...
      await db.prepare(
        "INSERT OR IGNORE INTO chat_participants (id, chat_id, user_id, role, joined_at) VALUES (?, ?, ?, 'member', ?)"
      ).bind(crypto.randomUUID(), groupId, memberId, Date.now()).run();
    }
  ...
  async listUserGroups(db, userId) {
  ...
  async getGroupMembers(db, groupId) {
    const res = await db.prepare(`
  ...
  async isGroupAdmin(db, groupId, userId) {
  ...
  async isGroupMember(db, groupId, userId) {
    const member = await db.prepare(
  ...
  async addGroupMember(db, groupId, userId, role = 'member') {
    const now = new Date().toISOString();
    await db.prepare(
    ...
    await db.prepare(
      "INSERT OR IGNORE INTO chat_participants (id, chat_id, user_id, role, joined_at) VALUES (?, ?, ?, ?, ?)"
    ).bind(crypto.randomUUID(), groupId, userId, role, Date.now()).run();

    return true;
  },

  async removeGroupMember(db, groupId, userId) {
    await db.prepare(
  ...
  // --- REACTIONS ---
  async toggleMessageReaction(db, messageId, userId, emoji) {
  ...
  async getMessageReactions(db, messageId) {
  ...
  // --- PINNING ---
  async pinMessage(db, messageId, userId) {
    const now = new Date().toISOString();
    await db.prepare(
  ...
  // --- BLOCK & MUTE ---
  async isUserBlocked(db, blockerId, blockedId) {
    const res = await db.prepare(
