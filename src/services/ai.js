 ...
 * @param {any} env
 * @param {string} text
 * @returns {Promise<{ si: string, ta: string, en: string }>}
 */
export async function autoTranslateMessage(env, text) {
  const result = {
    si: text,
    ta: text,
    en: text
  ...
  const targetLangs = ['si', 'ta', 'en'].filter(l => l !== detected);
  ...
  for (const target of targetLangs) {
    try {
      ...
      console.warn(`[ChatLK AI] Translation to ${target} note:`, err.message);
      result[target] = text;
    }
  }
 ...
 * @param {any} env
 * @param {string} chatId
 * @param {Array<{ text: string, sender_id: string }>} recentMessages
 * @param {string} userLang
 * @returns {Promise<string[]>}
 */
export async function getSmartReplySuggestions(env, chatId, recentMessages, userLang = 'en') {
  ...
  try {
    ...
    console.warn('[ChatLK AI] Suggestions generation note:', err.message);
    return fallback;
  }
}
 ...
 * @param {any} env
 * @param {string} text
 * @returns {Promise<{ status: 'clean'|'flagged'|'blocked', reason?: string, severity?: 'low'|'medium'|'high' }>}
 */
export async function moderateMessageText(env, text) {
  if (!text || typeof text !== 'string') {
    return { status: 'clean' };
  }

  // Fast-path heuristic for obvious severe violations
  ...
  try {
    ...
    console.warn('[ChatLK AI] Moderation run note:', err.message);
  }
